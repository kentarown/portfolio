<template>
  <div class="container mx-auto text-center my-8">
    <SectionTitle id="about-title" title="About"></SectionTitle>

    <section class="text-gray-600 body-font">
      <div class="container px-5 mx-auto flex flex-col">
        <div class="lg:w-4/6 mx-auto">
          <div class="flex flex-col sm:flex-row ">
            <div class="sm:w-1/3 text-center sm:pr-8 sm:py-1">
              <div class="w-20 h-20 rounded-full inline-flex items-center justify-center bg-gray-200 text-gray-400">
               <img src="/img/me.jpeg">
              </div>
              <div class="flex flex-col items-center text-center justify-center">
                <h2 class="font-medium title-font mt-4 text-gray-900 text-lg">kentarow</h2>
                <div class="w-12 h-1 bg-indigo-500 rounded mt-2 mb-4"></div>
                <p class="text-base">昭和生まれ＠九州在住</p>
              </div>
            </div>
            <div class="sm:w-2/3 sm:pl-8 sm:py-1 sm:border-l border-gray-200 sm:border-t-0 border-t mt-4 pt-4 sm:mt-0 text-center sm:text-left">
              <p class="leading-relaxed text-lg mb-4">web系のエンジニアを20年ほどやっています。平日日中はSierに所属し開発業務に携わっています。それなりに経験を積んできてるので、開発だけでなくマネジメント業務もぼちぼちと行ってきました。</p>
              <p class="leading-relaxed text-lg mb-4">以前から拠点間開発を通してリモートワークを行うことも多くあったのでコミュニケーションのあり方は特に大切にしています。</p>
            </div>
          </div>
        </div>
      </div>
    </section>
  </div>
</template>

<script>
import SectionTitle from "./parts/SectionTitle";
export default {
  name: "About",
  components: {SectionTitle}
}
</script>

<style scoped>

</style>
